...
# Repair a malformed outreach helper left by the previous generator before
# touching the render function. Refuse to alter anything else if the expected
# malformed signature is not present.
bad = "waHref=x=>{const n=phoneForWhatsApp(x.businessPhone);return n?'https://wa.me/'+n+'?text='+encodeURIComponfunction render(){"
good = "waHref=x=>{const n=phoneForWhatsApp(x.businessPhone);return n?'https://wa.me/'+n+'?text='+encodeURIComponent(outreachText(x,'whatsapp')):'#'},persist=()=>localStorage.setItem(KEY,JSON.stringify(leads));function render(){"
if bad in s:
...
print('Lead Finder upgraded: company grouping preserved, sales-fit scoring/filtering added, and malformed outreach helper repaired when present.')
