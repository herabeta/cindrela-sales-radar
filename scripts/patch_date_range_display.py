...
# Repair the accidental UX injection from the previous update before applying
# the normal date-range patches. This is deliberately idempotent.
p = ROOT / "index.html"
s = p.read_text(encoding="utf-8")
if "sales-radar-ux-v1" in s:
    css_start = s.find("<style>/* sales-radar-ux-v1 */")
    if css_start >= 0:
        css_end = s.find("</style>", css_start)
        if css_end >= 0:
            s = s[:css_start] + s[css_end + len("</style>"):]
    js_start = s.find("<script>/* sales-radar-ux-v1 */")
    if js_start >= 0:
